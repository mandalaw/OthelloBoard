...
public class OthelloBoard {
	 ...
	 * 
	 * @param character representing a player - should be either P1 or P2
	...
	private boolean validCoordinate(int row, int col) {
		if (row >= 0 && row < getDimension() && col >= 0 && col < getDimension()) {
			return true;
		}
		return false;
	}
	...
	private char checkForPattern(int row, int col, int drow, int dcol) {
		...
		while (validCoordinate(row0,col0) && validCoordinate(row,col)) {
			char theplayer = this.board[row][col];
			char current = this.board[row0][col0];
			if ((drow == 0 && dcol == 0) || theplayer == EMPTY || current == EMPTY){
				return EMPTY;}
			if (otherPlayer(theplayer) == current){
				return otherPlayer(theplayer);}
			col0 = col0 + dcol;
			row0 = row0 + drow;
			}
		return EMPTY;
	...
	private int flip(int row, int col, int drow, int dcol, char player) {
		...
		if (this.board[row][col] == player)
			return 0;
		if (checkForPattern(row, col, drow, dcol) == EMPTY || !validCoordinate(row, col)) {
			return -1;
		} else {
			while (this.board[row1][col1] == otherPlayer(player)) {
				this.board[row1][col1] = player;
				count++;
				row1 = row1 + drow;
				col1 = col1 + dcol;
			}
	...
	public boolean move(int row, int col, char player) {
		...
		if (!validCoordinate(row, col) || this.board[row][col] != EMPTY) {
			...
			if (validCoordinate(row, col) && flipped) {
